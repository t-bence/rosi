...
class TestMergeOverrides:
    ...
    def test_wav_file_and_tach_channel_override(self, tmp_path, mic_csv):
        wav = tmp_path / "test.wav"
        wav.write_bytes(
            b"RIFF\x24\x00\x00\x00WAVEfmt \x10\x00\x00\x00\x01\x00\x01\x00\x44\xac\x00\x00\x88\x58\x01\x00\x02\x00\x10\x00data\x00\x00\x00\x00"
        )
        p = _valid_yaml(tmp_path)
        cfg = load_config_from_yaml(p)
        merged = merge_config_with_overrides(
            cfg, {"wav_file": str(wav), "tach_channel": 2, "sources": [], "sample_rate": None, "duration": None}
        )
        assert merged.wav_file == str(wav)
        assert merged.tach_channel == 2
...
class TestRotationDirection:
    ...
    def test_accepts_valid(self, val, tmp_path, mic_csv):
        p = _valid_yaml(tmp_path, {"rotation_direction": val})
        cfg = load_config_from_yaml(p)
        assert cfg.rotation_direction == val

    @pytest.mark.parametrize("val", [0, 2, -2])
    def test_rejects_invalid(self, val, tmp_path, mic_csv):
        p = _valid_yaml(tmp_path, {"rotation_direction": val})
        with pytest.raises(ValidationError):
            load_config_from_yaml(p)

    def test_defaults_to_ccw(self, tmp_path, mic_csv):
        p = _valid_yaml(tmp_path)
        cfg = load_config_from_yaml(p)
        assert cfg.rotation_direction == 1


# ── tach_channel mode rules ────────────────────────────────────────────────────


class TestTachChannel:
    def test_tach_channel_without_wav_file_raises(self, tmp_path, mic_csv):
        p = _valid_yaml(tmp_path, {"tach_channel": 0})
        with pytest.raises(ValidationError, match="tach_channel requires wav_file"):
            load_config_from_yaml(p)

    def test_wav_file_without_rpm_or_tach_channel_raises(self, tmp_path, mic_csv):
        wav = tmp_path / "test.wav"
        wav.write_bytes(
            b"RIFF\x24\x00\x00\x00WAVEfmt \x10\x00\x00\x00\x01\x00\x01\x00\x44\xac\x00\x00\x88\x58\x01\x00\x02\x00\x10\x00data\x00\x00\x00\x00"
        )
        p = _valid_yaml(
            tmp_path,
            {"wav_file": str(wav), "sources": [], "sample_rate": None, "duration": None, "rpm": None},
        )
        with pytest.raises(ValidationError, match="rpm is required unless tach_channel is set"):
            load_config_from_yaml(p)

    def test_wav_file_with_tach_channel_and_no_rpm_is_valid(self, tmp_path, mic_csv):
        wav = tmp_path / "test.wav"
        wav.write_bytes(
            b"RIFF\x24\x00\x00\x00WAVEfmt \x10\x00\x00\x00\x01\x00\x01\x00\x44\xac\x00\x00\x88\x58\x01\x00\x02\x00\x10\x00data\x00\x00\x00\x00"
        )
